...
function inject_current_ticket_number() {
  ...
  inject_text_into_buffer $(current_branch | sed -E 's/.*\/([A-Z]+-[0-9]+)_.*$/\1/')
}

function inject_current_first_ticket_number() {
  inject_text_into_buffer $(current_branch | sed -E 's/.*\/([A-Z]+-[0-9]+).*/\1/')
}

zle -N inject_current_branch
zle -N inject_current_ticket_number
# zle -N inject_current_first_ticket_number
bindkey "\C-b" inject_current_branch
bindkey "\C-n" inject_current_ticket_number
# bindkey "\C-m" inject_current_first_ticket_number
